...
export function CrudPage<T extends { id: string | number }>({
  ...
  const openEdit = (row: T) => {
    ...
    const derived = edit.toInitialValues
      ...
      : (Object.fromEntries(edit.fields.map((f) => [f.name, String((row as any)[f.name] ?? "")])) as Record<
          string,
          string
        >);
    ...
    <div className="space-y-6">
                ...
                {create!.fields.map((f) => (
                  <div key={f.name} className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setCreateValues((s) => ({ ...s, [f.name]: e.target.value }))}
                    />
                  </div>
                ))}
              </div>
      ...
      <div className="glass rounded-2xl">
        <div className="p-4 border-b border-border/40 flex items-center gap-2">
          ...
          <Badge variant="outline">{loading ? "Loading…" : `${filtered.length} of ${rows.length}`}</Badge>
        </div>
                    ...
                    >
                      ...
                      <DialogContent>
                          ...
                          {edit!.fields.map((f) => (
                            <div key={f.name} className="grid gap-2">
                              ...
                              <Input
                                ...
                                onChange={(e) => setEditValues((s) => ({ ...s, [f.name]: e.target.value }))}
                              />
                            </div>
                          ))}
                        </div>
                        <DialogFooter>
                          ...
                          <Button className="gradient-brand text-white border-0" onClick={submitEdit}>
                            Save
                          </Button>
                        </DialogFooter>
                      </DialogContent>
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction onClick={() => submitDelete(r.id)}>Delete</AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
                  )}
